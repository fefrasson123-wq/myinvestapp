...
export function useGoldPrice() {
  ...
  const fetchPrice = useCallback(async () => {
    ...
    try {
      // Tenta buscar preço via CoinGecko (PAX Gold = 1 onça de ouro) com rate limiting
      const result = await coingeckoRateLimiter.execute(async () => {
      ...
      if (result['pax-gold']?.brl) {
        const pricePerOunce = result['pax-gold'].brl;
